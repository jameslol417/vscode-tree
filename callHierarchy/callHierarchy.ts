...
import { ProviderResult } from './common/languages.js'
...
import { CommandsRegistry } from '../../../../platform/commands/common/commands.js'; // TODO?: Command Task Line for VSCode
...
export class CallHierarchyModel {
	...
	async resolveIncomingCalls(item: CallHierarchyItem, token: CancellationToken): Promise<IncomingCall[]> {
		try {
			...
			if (Array.isArray(result) && result.length > 0) { // export function isNonEmptyArray<T>(obj: T[] | undefined | null): obj is T[]
				return result;
			}
		} catch (e) {
