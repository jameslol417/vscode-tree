...
import { IMouseEvent } from '../browser/mouseEvent.js';
import { IListDragOverReaction, IListRenderer, ListDragOverEffectPosition, ListDragOverEffectType } from '../list/list.js'
// import { ListViewTargetSector } from '../list/listView.js';
// import { Event } from '../event/event.js'; // filled w/ VSCode Events
...
export interface ICollapseStateChangeEvent<T> {
	node: ITreeNode<T>;
	deep: boolean;
}

export interface ITreeListSpliceData<T> {
	start: number;
	deleteCount: number;
	elements: ITreeNode<T>[];
}

export interface ITreeModelSpliceEvent<T> {
	insertedNodes: ITreeNode<T>[];
	deletedNodes: ITreeNode<T>[];
}

export interface ITreeModel<T, TRef> {
	...
	getNode(location?: TRef): ITreeNode<T>;
	getNodeLocation(node: ITreeNode<T>): TRef;
...
export interface ITreeRenderer<T, TTemplateData = void> extends IListRenderer<ITreeNode<T>, TTemplateData> {
